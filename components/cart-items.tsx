...
interface CartItemsProps {
  ...
  setCartItems: (items: SanityProduct[]) => void;
}

export function CartItems({ cartItems, setCartItems }: CartItemsProps) {

  const { toast } = useToast();
  // @ts-ignore
  const { handleRemoveCartItem } = useGlobalContext()
  ...
  const setItemQuantity = (id: string, givenQuantity: number) => {
    let neuCartItems = cartItems.filter((cartItem) => {
      if (cartItem._id === id)
        cartItem.quantity = givenQuantity
      return cartItem
    })
